...
def make_dataset_from_rlds(
    ...
    *,
    ...
    def restructure(traj):
        # 添加调试信息
        print(f"[DEBUG] Processing dataset: {name}")
        print(f"[DEBUG] Expected state_obs_keys: {state_obs_keys}")
        ...
        # 添加更多调试信息
        print(f"[DEBUG] Available observation keys: {list(old_obs.keys())}")
        ...
        if state_obs_keys:
            print(f"[DEBUG] Trying to access state keys: {state_obs_keys}")
            for i, key in enumerate(state_obs_keys):
                if key is not None and key not in old_obs:
                    print(f"[ERROR] Missing key '{key}' in dataset '{name}'. Available keys: {list(old_obs.keys())}")
                    raise KeyError(f"Dataset '{name}' missing observation key: '{key}'")
...
def apply_trajectory_transforms(
    ...
    *,
    ...
    dataset = dataset.filter(lambda traj: tf.shape(traj["action"])[0] > future_action_window_size)# fractual need https://github.com/openvla/openvla/issues/239#issuecomment-2791359272 

    dataset = dataset.traj_map(
        partial(
            # traj_transforms.chunk_act_obs,
            traj_transforms.chunk_act_obs_no_padding,
    ...
    for dataset_kwargs, threads, reads in zip(
        ...
        dataset = apply_trajectory_transforms(
            ...
            **traj_transform_kwargs,
            num_parallel_calls=threads,
            train=train,
        ).flatten(num_parallel_calls=threads) #在这里分action chunk

        def filter_nonempty(traj):
            return tf.shape(traj["action"])[0] > 0
        dataset = dataset.filter(filter_nonempty)
