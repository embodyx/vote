# copy from https://github.com/microsoft/CogACT/blob/main/action_model/models.py
...
class DiT(nn.Module):
    ...
    def forward(self, x, t, z):
        ...
        """
        # Adapt batch size of z to match x if they are different but compatible
        if z.shape[0] != x.shape[0] and x.shape[0] % z.shape[0] == 0:
            z = z.repeat(x.shape[0] // z.shape[0], 1, 1)
        # x torch.Size([400, 16, 7])
        x = self.x_embedder(x)                              # (N, T, D)
        # x torch.Size([400, 16, 384])
        # t  torch.Size([400])
        t = self.t_embedder(t)                              # (N, D)
        #  t torch.Size([400, 384])
        #z 应该是1 , 不应该是16.
