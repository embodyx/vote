...
def log_gpu_memory(tag="", reset_max=False):
    """
    Log current GPU memory usage.
    
    Args:
        tag: Label for the logging point
        reset_max: Whether to reset the peak memory stats
    """
    ...
    print(f"GPU Memory [{tag}]: {allocated:.2f}GB (Peak: {max_allocated:.2f}GB, Reserved: {reserved:.2f}GB)")
...
def get_lora_model(cfg: Any) -> torch.nn.Module:
    ...
    base_vla = OpenVLAForActionPrediction.from_pretrained(
        cfg.base_vla_path, torch_dtype=torch.bfloat16, low_cpu_mem_usage=True, trust_remote_code=True
    )
    base_vla.vision_backbone.set_num_images_in_input(cfg.num_images_in_input)
    if cfg.use_film:
        base_vla = _apply_film_to_vla(base_vla, cfg)
    
    # 需要把一切都先加载到base vla,不能对merge vla操作上面这些.     
    vla_lora = PeftModel.from_pretrained(base_vla, cfg.pretrained_checkpoint, subfolder="lora_adapter", use_safetensors=True) 
    ...
    # Move model to device if not using quantization
    if not cfg.load_in_8bit and not cfg.load_in_4bit:
        vla_lora = vla_lora.to(DEVICE)

    _load_dataset_stats(vla_lora, cfg.pretrained_checkpoint) # Load dataset stats for action normalization
    return vla_lora
...
def get_action_head(cfg: Any, llm_dim: int) -> torch.nn.Module:
    """
